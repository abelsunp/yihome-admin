    ...
    <el-col :xs="12" :sm="12" :lg="6" class="card-panel-col">
      <div class="card-panel" @click="handleSetLineChartData('newVisitis')">
        ...
        <div class="card-panel-description">
          ...
          <count-to :start-val="0" :end-val="needHandel" :duration="2600" class="card-panel-num" />
        </div>
      </div>
    </el-col>
    <el-col :xs="12" :sm="12" :lg="6" class="card-panel-col">
...
import { listItem } from "@/api/handle/matter";
export default {
  components: {
    CountTo
  },
  ...
  created(){
    listItem().then(response => {
      this.needHandel = response.rows.length
    });
  },
  methods: {
    handleSetLineChartData(type) {
      if(type=='newVisitis'){
        this.$router.push('/handle/matter')
      }
