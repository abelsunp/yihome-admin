<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        ...
        <el-button
          ...
          icon="el-icon-share"
          size="mini"
          :loading="ceratedStatus"
          @click="handleCreated"
          v-hasPermi="['user:users:createdLink']"
        >生成连接</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="usersList" @selection-change="handleSelectionChange" @cell-click="cellClick">
      ...
      <el-table-column label="姓名" align="center" prop="nameCh" fixed class-name="same-color" :show-overflow-tooltip="true"/>
      ...
      <el-table-column label="操作" align="center" class-name="same-color" fixed="right" width="370">
        <template slot-scope="scope">
          <el-button
            size="mini"
            type="text"
          ...
          <el-button
            ...
            :loading="scope.row.updateLinkStatus"
            v-hasPermi="['user:users:updateLink']"
          >生成编辑连接</el-button>
          ...
          <el-button
            ...
            v-hasPermi="['user:users:distribution']"
          >分配</el-button>
          <el-button
            size="mini"
            type="text"
    ...
    <el-drawer
      ...
      <div class="user-wrapper" v-if="addUserDrawer">
        <div class="user-container">
          <Add-user @changeStatus="updata"  :user-data="currentData" user-owner="user"/>
        </div>
      </div>
    </el-drawer>
    ...
    <el-drawer
      ...
      <div v-if="recordStatus">
        <follow-record :user-data="currentRowData" :active-status="activeStatus" :channel-data="channelData" :level-data="userStatusDataChilderLevel"/>
      </div>
    </el-drawer>
...
<script>
import { listUsers, getUsers,userAllocation,getUserByRole,allSalesList,userUpdateLink, userCreateLink } from "@/api/user/users";
...
export default {
  ...
  created() {
    ...
    if(this.$route.query.nameCh){
      this.queryParams.nameCh = this.$route.query.nameCh;
    }
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      allSalesList(this.queryParams).then(response => {
    ...
    resetQuery() {
      ...
      this.handleQuery();
      this.queryParams.nameCh = undefined;
    ...
    //生成连接
    handleCreated(){
      this.ceratedStatus = true;
      userCreateLink().then(res=>{
        this.ceratedStatus = false;
        if(res.code==200){
          ...
          if(process.env.NODE_ENV === "production"){
            input.value = window.origin+'/student?key='+res.data.userId+'&uuid='+res.data.uuid;   // 设置内容

          }else{
            input.value ='https://www.inyihome.com:6898/student?key='+res.data.userId+'&uuid='+res.data.uuid;   // 设置内容
          }
    ...
    handleUpdateLink(row){
      row.updateLinkStatus = true;
      userCreateLink().then(res=>{
        row.updateLinkStatus = false;
        if(res.code==200){
          var input = document.createElement("input");     // 直接构建input
          let url = process.env.VUE_APP_BASE_API;

          if(process.env.NODE_ENV === "production"){
            input.value = window.origin+'/student?key='+res.data.userId+'&userId='+row.id+'&uuid='+res.data.uuid;   // 设置内容
          }else{
            input.value = 'https://www.inyihome.com:6898/student?key='+res.data.userId+'&userId='+row.id+'&uuid='+res.data.uuid;   // 设置内容
          }
      ...
      /*userUpdateLink({userId:row.id}).then(res=>{

        console.log(res)
        if(res.code==200){
          var input = document.createElement("input");     // 直接构建input
          input.value = res.msg;   // 设置内容
          document.body.appendChild(input);        // 添加临时实例
          input.select();      // 选择实例内容
          document.execCommand("Copy");     // 执行复制
          document.body.removeChild(input);// 删除临时实例
