<template>
  ...
  <section class="addHouseW">
    ...
    <div>
      ...
      <el-button style="height:32px;" size="mini" @click="handleHouseClose2(1)">返回供应商</el-button>
    </div>
    ...
    <div style="background-color:#fff;padding-top:32px;border-radius: 4px;">
      <el-steps :active="stepActive"  align-center>
        ...
        <el-step v-if="$route.query.supplierId" title="合同信息" @click.native="on_step(4)" />
        <el-step title="添加单间" @click.native="on_step(5)" />
      </el-steps>
        ...
        <el-form ref="addHouseForm" :model="addHouseForm" :rules="addHouseRules" label-width="150px" class="addHouseForm">
          <div v-show="stepActive==1" style="max-width: 800px;margin-top:30px;">
            ...
            <el-form-item label="房源名称(中文):" prop="name">
              <el-input ref="customerInput" v-model="addHouseForm.name" placeholder="请输入房源名称(中文)" clearable/>
            </el-form-item>
            <el-form-item label="房源名称(英文):" prop="nameEn">
              <el-input v-model="addHouseForm.nameEn" placeholder="请输入房源名称(英文)" clearable />
            </el-form-item>
            ...
            <el-form-item label="供应商：" prop="providerId">
              <el-select v-model="addHouseForm.providerId" disabled filterable placeholder="请选择或搜索供应商" style="width: 100%;" clearable>
                <el-option
            ...
            <el-form-item label="地址：" prop="address">
              <el-input v-model="addHouseForm.address"  clearable placeholder="请输入地址"/>
            </el-form-item>

            <!-- 新增 -->
            <el-form-item label="beds数量：" prop="beds">
              <el-input v-model="addHouseForm.beds"  clearable placeholder="请输入beds数量"/>
            </el-form-item>
          ...
          <div v-show="stepActive==3&&$route.query.supplierId" style="max-width: 900px;text-align:center;margin:auto;margin-top:32px;">
            <el-form :model="processForm"  ref="processForm" label-width="215px">
                    ...
                    <el-form-item label="可否选房" prop="isSelectd">
                        <el-select v-model="processForm.isSelectd" placeholder="请选择" style="width:100%;" clearable>
                    ...
                    <el-form-item label="是否选择楼层" prop="floorSelect">
                        <el-select v-model="processForm.floorSelect" placeholder="请选择" style="width:100%;" clearable>
                            <el-option label="是" value="1"></el-option>
                            <el-option label="否" value="2"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="是否可以知道室友性别" prop="chumSexSelect">
                        <el-select v-model="processForm.chumSexSelect" placeholder="请选择" style="width:100%;" clearable>
                            <el-option label="是" value="1"></el-option>
                            <el-option label="否" value="2"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="团队订房是否可安排住同一个flat" prop="flatSelect">
                        <el-select v-model="processForm.flatSelect" placeholder="请选择" style="width:100%;" clearable>
                            <el-option label="是" value="1"></el-option>
                            <el-option label="否" value="2"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="是否接受紧急入住" prop="urgencySelect">
                        <el-select v-model="processForm.urgencySelect" placeholder="请选择" style="width:100%;" clearable>
                            <el-option label="是" value="1"></el-option>
                            <el-option label="否" value="2"></el-option>
                        </el-select>
                    </el-form-item>
          ...
          <div v-show="stepActive==4&&$route.query.supplierId" style="max-width: 900px;text-align:center;margin:auto;margin-top:32px;">
            <el-form :model="contractForm" ref="contractForm" label-width="180px">
                    ...
                    <el-form-item label="付款计划" prop="paymentPlan">
                        <vue-ueditor-wrap v-model="contractForm.paymentPlan" :config="myConfig" :key="91"></vue-ueditor-wrap>
                    </el-form-item>
                    <el-form-item label="佣金方案" prop="commission">
                        <vue-ueditor-wrap v-model="contractForm.commission" :config="myConfig" :key="41"></vue-ueditor-wrap>
                    </el-form-item>

                    <el-form-item label="佣金支付周期" prop="paymentCycle">
                        <vue-ueditor-wrap v-model="contractForm.paymentCycle" :config="myConfig" :key="51"></vue-ueditor-wrap>
                    </el-form-item>

                    
                    <el-form-item label="需缴纳的款项金额" prop="paymentAmount">
                        <vue-ueditor-wrap v-model="contractForm.paymentAmount" :config="myConfig" :key="61"></vue-ueditor-wrap>
                    </el-form-item>
      ...
      <div v-show="stepActive!=5" class="buttonWrapper">
        <el-button type="primary" size="small" :disabled="stepActive==5" @click="next()">下一步</el-button>
...
import {listSupplier, addSupplierContact,updateSupplierContact,getSupplierContact,delSupplierContact,getContactDetails,getSupplierSyncList,getContract  } from "@/api/supplier";

import { listCommission,addCommissionMethods,getCommissionMethods,updateCommission,addHouseSync,listHouseSync } from "@/api/commission/commission";
...
export default {
  ...
  methods: {
    ...
    handleHouseClose(val) {
      ...
      this.$router.push({ path: "/rent/house", query: { t: Date.now()}})
      this.addHouseForm.id = undefined
      this.$emit('on-result-change', false)
      ...
      if (this.stepActive == 1) {
        ...
        if(this.$route.query.supplierId){
          ...
          this.processForm.id = undefined;
            addHouseSync(this.processForm).then(res=>{
              this.commissionStatus = false;
              if(res.code===200){
                this.stepStatus()
                //获取合同信息
                getContract({supplierId:this.$route.query.supplierId}).then(restow=>{
                  if(restow.code===200){
                    if(restow.rows.length!=0){


                      const sss = []
                      if(restow.rows[0].paymentType){
                        restow.rows[0].paymentType =restow.rows[0].paymentType.split(',');
                        
                        restow.rows[0].paymentType.forEach(el=>{
                            sss.push(parseInt(el));
                        })
                      }
                      console.log(sss)

                      this.contractForm = restow.rows[0];
        ...
        if(this.$route.query.supplierId){
          this.contractForm.houseId = this.dataparams.houseId;
          this.contractForm.id = undefined;
          if(this.contractForm.paymentType){
            this.contractForm.paymentType = this.contractForm.paymentType.join()
          }
          addCommissionMethods(this.contractForm).then(res=>{
            if(res.code===200){
                this.stepStatus()
              }else{
                  this.msgError(res.msg)
    ...
    getContactList(){
      getSupplierContact({masterId:this.dataparams.houseId,type:2}).then(res=>{
